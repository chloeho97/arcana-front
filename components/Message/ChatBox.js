...
const ChatBox = ({ currentUserId, targetUserId, targetUser, onCloseChat }) => {
  ...
  const fetchMessages = async () => {
    try {
      const res = await fetch(
        `https://arcana-back-two.vercel.app/messages/${currentUserId}/${targetUserId}`
      );
      const data = await res.json();
      setMessages(data);

      await fetch(
        `https://arcana-back-two.vercel.app/messages/${currentUserId}/${targetUserId}/mark-read`,
        {
          method: "POST",
  ...
  const handleSend = async () => {
    ...
    try {
      await fetch("https://arcana-back-two.vercel.app/messages", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleDeleteMessage = async () => {
    ...
    try {
      await fetch(
        `https://arcana-back-two.vercel.app/messages/${selectedMessage._id}`,
        {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
