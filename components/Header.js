      ...
      if (user.userId) {
        try {
          const userResponse = await fetch(
            `https://arcana-back-two.vercel.app/users/${user.userId}`
          );
    ...
    const checkUnreadMessages = async () => {
      try {
        const response = await fetch(
          `https://arcana-back-two.vercel.app/messages/${user.userId}/unread/total`
        );
  ...
  const handleLogout = async () => {
    ...
    try {
      const { data } = await axios.post(
        "https://arcana-back-two.vercel.app/users/logout",
        {
          token,
        }
      );
