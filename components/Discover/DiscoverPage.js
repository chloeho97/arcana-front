...
export default function Discover() {
  ...
  const fetchCollections = async () => {
    try {
      const [topRes, recentRes, randomRes] = await Promise.all([
        axios.get("https://arcana-back-two.vercel.app/collections/top"),
        axios.get("https://arcana-back-two.vercel.app/collections/recent"),
        axios.get("https://arcana-back-two.vercel.app/collections/random"),
      ]);
  ...
  const fetchUserLikes = async () => {
    try {
      ...
      const { data } = await axios.post(
        "https://arcana-back-two.vercel.app/users/by-token",
        {
          token,
        }
      );
  ...
  const handleToggleLike = async (id) => {
    try {
      ...
      if (already) {
        await axios.delete(
          `https://arcana-back-two.vercel.app/likes/${userId}/${id}`
        );
        setLikedIds((prev) => prev.filter((x) => x !== id));
      } else {
        await axios.post("https://arcana-back-two.vercel.app/likes/like", {
          userId,
          collectionId: id,
        });
